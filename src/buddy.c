...
void kmem_test_b (void)
{
    ...
    for (i = 0; i < 50; i++) {
        j *= 1237;
        blocks[i].order = j % (N_ORD -1) + MIN_ORD;
        blocks[i].mem = kmalloc(blocks[i].order);

        for (k = 0; k < (1<<blocks[i].order); k++) {
            if (((char*)(blocks[i].mem))[k] != 0) {
                panic("memory error\n");
            }

            ((char*)(blocks[i].mem))[k] = (char)i;
        }
    }

    for (i = 25; i >= 0; i--) {
        memset(blocks[i].mem, 0, (1<<blocks[i].order));
        kfree(blocks[i].mem, blocks[i].order);        
    }
